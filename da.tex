...
\subsection{Optimal Tree for fixed flows}
...
\end{algorithm}
In a nutshell, this algorithm performs at every branching point the search for the $n_c$ scenarios that best cover the distribution given by all scenarios that the current node in the previous stage was used to cover (figure?).

The algorithm is presented outlined as algorithm \ref{alg:stage-wise-milp}. The algorithm is provided with a set of scenarios $I$ and a set of stages $T$. The distances $c(\xi_i^t, \xi_j^t)$ between the scenarios $\xi_i,\,\xi_j\; i,j\in I$ can be computed beforehand. Note that the $1$-norm is the only meaningful norm that can be constructed this way, since the full distance is effectively approximated as a sum over the stages. 
\todo[inline]{Describe each step of the algorithm im Freitext}

      \begin{eqnarray}
        \label{eq:small-milp-in-alg}
        \mathcal{M}_{sw}(f)\min_{\eta^f}&&\sum_{i\in I_f}\sum_{j\in I_f}\eta_{ij}^fc(\xi_i^t,\xi_j^t)\\
