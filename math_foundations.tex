...
\end{equation}
where $D$ is an appropriate norm. See \citeasnoun{Heitsch2010} for the proof and further details.
...
Besides its virtues for the stability analysis (see \citeasnoun{Dupacova2003}), an advantage of the Kantorovich Distance is its simple and intuitive representation for \textit{discrete} random variables.
...
See \citeasnoun{Heitsch2010} an introduction to how this can be done.
Instead, the filtration will be postulated beforehand.
The complexity of a stochastic programming problems, which can be measured by the number of variables in the deterministic equivalent\footnote{see \citeasnoun{Birge1997} for an overview of deterministic equivalents and other solution methods for stochastic programs}, is fully determined by the filtration tree of the scenario tree of $\bs\chi$.
...
Since both $\bs\xi$ and $\bs\chi$ are discrete stochastic processes, the Kantorovich Distance is equivalent to the minimum cost flow problem described above (\ref{eq:define-finitedim-Kantorovich}).
