...
\citeasnoun{Pardalos1991} showed that quadratic optimization problems with at least one negative eigenvalue are \textbf{NP}-hard.
...
The reason for the poor local convergence again lies in the structure of the problem. \citeasnoun{Waechter2005} state that in order to be able to achieve superlinear local convergence, the sufficient second order conditions (SSOC) must hold for the optimal point.
