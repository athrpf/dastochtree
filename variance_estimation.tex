...
\begin{figure}
  \centering
    \begin{tikzpicture}[node distance=5cm, auto]
      ...
      \node [block, right of=infsolver] (xorig){$f^{\mathrm{opt}}(\xi)$};
      \node [block, text width=10em, below of=infsolver] (nlp) {(N)LP Solver};
      \node [block, below of=xorig] (x) {$f^{\mathrm{opt}}(\hat{\xi})$};

      \path [line] (xiorig) -- node (xierror) {$\sigma$} (xi);
      \path [line] (x) -- node (xerror) {$\operatorname{Var}\off{f^{\mathrm{opt}}(\hat{\xi})}$} (xorig);
