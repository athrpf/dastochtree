...
\]
Each mean value $\mu_n$ (and its corresponding variance) can be uniquely identified with a specific node of the tree. This makes it possible to enforce the tree structure.
\input{em_tree}
...
In the expectation step, the probabilities $p(z_{nk}|\xi_n, \mu_k,\sigma_k,\pi_k)$ are computed.
For this step, the scenarios $\xi_i$ of the sample set are regarded as data vectors, while the 
...
The Taylor expansion of optimal value $f(x^*,\xi)$ with respect to the node values at $\xi_0$ is
\begin{equation}
  \label{eq:47}
  f(x^*(\xi),\xi) \approx f(x^*(\xi_0),\xi_0) + \nabla_\xi^T \mathcal{L}(x,\xi)(\xi-\xi_0) 
\end{equation}
with $\mathcal{L}$ the Lagrangean of the above optimization problem (\cite{Jongen2004}, theorem 3.2.3).
\input{variance_estimation}

Suppose now that $\xi_0$ was generated using the Mixture of Gaussians EM-algorithm of the previous section.
...
The variance of the objective value with respect to the node values $\xi$ is defined as the Kantorovich distance between the distribution of scenarios optimal values over scenarios and the optimal solution
\begin{equation}
  \label{eq:50}
  \mathrm{Var}_{\xi}\left[f^{\mathrm{opt}}\right] :=  \sum_{k=1}^K\int_{\mathbb{R}^T} \left(f_k^{\mathrm{opt}}(\xi)-f^{\mathrm{opt}}(\mu_k)\right)^2p_k(\xi)d\xi
\end{equation}
...
\end{equation}
It is assumed that the variances $\sigma_k$ are sufficiently small compared to the region in which \eqref{eq:47} holds.
The integral \eqref{eq:50} can be evaluated as
\begin{equation}
  \label{eq:52}
  \operatorname{Var}_\xi\left[f^{\mathrm{opt}}\right] = \sum_{k=1}^K\int_{\mathbb{R}^T} (\nabla_{\xi_k}^T\mathcal{L}(x^*(\xi_k),\xi_k)(\xi-\xi_k))^2\pi_k\mathcal{N}(\xi|\mu_k,\sigma_k)d\xi
\end{equation}
...
\begin{equation}
  \label{eq:54}
  \operatorname{Var}_\xi\left[f^{\mathrm{opt}}\right] = \sum_{k=1}^K \left(\nabla_{\xi_i} \mathcal{L}\right)^2\pi_k|\sigma_k| .
\end{equation}
The sensitivities are usually easily obtained from the solution of the optimization problem.
